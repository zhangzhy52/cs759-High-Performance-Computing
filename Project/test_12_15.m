...
numofLoop = 0;
 ...
 while( ~done)  
   ...
   k  = gpuArray(zeros(length(index), n));
   for i = 1: length(index)
    ...
    gw = gpuArray(zeros(length(index), n));
    gmu = gpuArray(zeros(length(index), n));
   
    %getE returns n*1;
    for i = 1:length(index)
        [gmu(i,:), gOmega] = gpufmuOmegaVector(k(i,:),w,mu,T);
        gmu(i,:) = gmu(i,:)./ gOmega;
...
%    end
 ...
 %stepSize2 = 0.0001;
  ...
  % mu = proj(mu + stepSize1 *  gmu' * (tmpf(index) - fwant)) ;
      mu = (mu + stepSize1 *  gmu' * (tmpf(index) - fwant)) ;
      mu = gather(mu);
