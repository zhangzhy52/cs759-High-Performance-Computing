...
float computeOnDevice(float* h_data, int num_elements)
{
	...
	int block_per_grid = num_elements / thread_per_block;

	float *hout = (float*) malloc(thread_per_block * sizeof(float));

	cudaMalloc( (void**) &dA, sizeof(float) * num_elements);
	cudaMalloc( (void**) &dout, sizeof(float) * thread_per_block);
	
	// TODO copy data from host to GPU 
	cudaMemcpy (dA, h_data, sizeof(float) * num_elements, cudaMemcpyHostToDevice);
	cudaMemset( dout, 0, thread_per_block *sizeof(float));
	
	reduction<<< block_per_grid, thread_per_block >>>(dA,  dout, num_elements);

	cudaMemcpy(hout, dout, sizeof(float) * thread_per_block, cudaMemcpyDeviceToHost);
	// placeholder
	float sum = 0;
