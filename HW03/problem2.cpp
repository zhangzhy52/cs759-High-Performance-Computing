...
int main() {
	...
	std::vector<point3Dchar> y(size);
	std::vector<point3DcharArray> z(size);
	...
	auto f2 = [&y, &sum]() {
		...
		sum = std::accumulate(y.begin(), y.end(), 0.0f,
				
				[](float total, point3Dchar const& p){ return total + p.x;});
	...
	auto f3 = [&z, &sum]() {
		...
		sum = std::accumulate(z.begin(), z.end(), 0.0f,
				
				[](float total, point3DcharArray const& p){ return total + p.x;});
	...
	sw.time_it(10UL, f2, min_time);
	sw.time_it(10UL, f3, min_time);
}
